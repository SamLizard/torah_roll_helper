...
making sure the locale code is valid.
...
(locale file name, flag file name, and locale id).
...
translated too. If it is **not** in the list, your language still works; only
Vuetify's internal strings fall back to English.
...
This part is mostly automatic.

The app loads locale files automatically from `src/locales/`, and the Vue i18n
setup merges app translations with Vuetify translations in
[`src/plugins/i18n.ts`](../../src/plugins/i18n.ts).

In practice, contributors usually do **not** need to edit `src/plugins/i18n.ts`.
The only time you need extra metadata is when:

* the language is right-to-left, or
* Vuetify uses a different export name than the locale id.

That special-case data lives in [`src/plugins/localeMeta.ts`](../../src/plugins/localeMeta.ts).

Examples:

* `he` works with the same key everywhere.
* `fa` also works with the same key everywhere.
* `zh-Hans` needs a Vuetify export name of `zhHans`.
* `sr-Cyrl` needs a Vuetify export name of `srCyrl`.

If Vuetify ships your language, its own UI strings (date pickers, dialogs, etc.)
are translated automatically. If it does **not** ship your language, the app
still works — Vuetify's internal strings fall back to English.

## 4. Set RTL if needed

If your language is right-to-left (Arabic, Hebrew, Farsi, etc.), add it to
[`src/plugins/localeMeta.ts`](../../src/plugins/localeMeta.ts) and set:

```ts
he: { rtl: true }
```

For left-to-right languages you do not need to add anything.
